...
void Terrain::CreateTriangles(int i1, int i2, int i3) {
	Triangle* tem_tri = new Triangle(i1, i2, i3, mVertices);
		...
		if (Triangles[c]->havechild()) {
			for (int i = 0; i < Triangles[c]->PotentialChildren.size(); i++) {
				...
				if (bay.y == 1 - bay.x) { //ligg på ab
					Triangle* te = new Triangle(Triangles[c]->point[2], Triangles[c]->point[0], Triangles[c]->PotentialChildren[i], mVertices);
					...
					e = false;
					Triangle* tem = new Triangle(Triangles[c]->point[1], Triangles[c]->point[2], Triangles[c]->PotentialChildren[i], mVertices);
					for (int d = 0; d < Triangles.size(); d++) {
					...
					Triangle* te = new Triangle(Triangles[c]->point[0], Triangles[c]->point[1], Triangles[c]->PotentialChildren[i], mVertices);
					for (int d = 0; d < Triangles.size(); d++) {
						if (Triangles[d]->center().GetPosition() == te->center().GetPosition() && Triangles[d]->rad() == te->rad()) {
							e = true;
						}
					...
					Triangle* tem = new Triangle(Triangles[c]->point[2], Triangles[c]->point[0], Triangles[c]->PotentialChildren[i], mVertices);
					for (int d = 0; d < Triangles.size(); d++) {
						if (Triangles[d]->center().GetPosition() == tem->center().GetPosition() && Triangles[d]->rad() == tem->rad()) {
							e = true;
						}
					...
					Triangle* t = new Triangle(Triangles[c]->point[2], Triangles[c]->point[1], Triangles[c]->PotentialChildren[i], mVertices);
					for (int d = 0; d < Triangles.size(); d++) {
